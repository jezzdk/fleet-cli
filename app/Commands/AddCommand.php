...
class AddCommand extends Command
{
    ...
    public function handle(Filesystem $filesystem): int
    {
        ...
        if (! file_exists('docker-compose.yml') && ! file_exists('docker-compose.backup.yml')) {
            $this->info('No docker-compose.yml file available, running sail:install...');
            Fleet::process('php artisan sail:install');
        }
        ...
        if (file_exists('docker-compose.yml')) {
            if ($this->confirm('This will modify your docker-compose.yml file, do you want to back it up first?')) {
                rename('docker-compose.yml', 'docker-compose.backup.yml');
            }
        }
        ...
        $yaml = $this->generateYamlForDockerCompose($file, $domain);
        ...
        if ($this->option('ssl')) {
            ...
            try {
                ...
                $this->error($e->getMessage());
                $this->line('For more information, try checking out the documentation at https://mkcert.dev');
        ...
        file_put_contents('docker-compose.yml', Yaml::dump($yaml, 6));
    ...
    private function generateYamlForDockerCompose(string $file, string $domain): array
    {
    ...
    private function isSailInstalled(): bool
    {
        ...
        return Arr::has($composer, 'require-dev.laravel/sail');
    }
}
